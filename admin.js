...
class AdminPanel {
    constructor() {
        this.apiUrl = 'https://script.google.com/macros/s/AKfycbyNgIlCy-kp8eEJa5znqOqLCHe_3csp7i4q21UDQ0VXB97v9W-m0G_1TMWmTteWmT3w/exec';
        this.menuData = [];
        this.ordersData = [];
    ...
    async syncMenuToSheets() {
        try {
            const response = await fetch(`${this.apiUrl}?action=syncMenu`, {
                method: 'POST',
                headers: {
            ...
            if (!result.success) {
                throw new Error('Failed to sync menu data');
            }

            console.log('Menu data synced successfully');
        } catch (error) {
            console.error('Error syncing menu data:', error);
            alert('Failed to sync menu data. Please try again.');
        }
    }
}
